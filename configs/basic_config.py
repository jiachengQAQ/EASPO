...
def basic_config():
    ...
    pretrained.model = "Grad-TTS/checkpts"
    config.use_lora = True
    config.lora_rank = 4
    
    ###### Preference Model ######
    config.preference_model_func_cfg = dict(
        type="emotion_preference_model_func",
        model_pretrained_model_name_or_path='Grad-TTS/checkpts',
        processor_pretrained_model_name_or_path='clep/CLEP-PANN-14',
        ckpt_path='model_ckpts/emotional_step-aware_preference_model.bin',
    )
    ...
    config.dataset_cfg = dict(
        ...
        pretrained_tokenzier_path='clep/CLEP-PANN-14',
    )
    ...
    #### validation ####
    config.validation_prompts = ['This is a happy sound']
    config.num_validation_speech = 2
    config.eval_interval = 1
    
    #### logging ####
    # run name for wandb logging and checkpoint saving.
    config.run_name = ""
    config.wandb_project_name = 'easpo'
